...
/* ── Icon components for watch home grid ── */
const VolumeIcon = () => (
...
const APP_GRID: AppIcon[] = [
  { id: 'app', label: 'Vestel', route: '/home', Icon: () => <VestelLogo />, isMainApp: true },
...
export default function WatchHomePage() {
  ...
  return (
    <div className="flex flex-col h-full bg-primary">
      {/* Apple Watch grid layout */}
      <div className="flex-1 grid grid-cols-3 gap-4 p-4 place-items-center content-center">
        {APP_GRID.map(({ id, label, route, Icon, isMainApp }) => (
          <button
            key={id}
            ...
            aria-label={label}
          >
            <div className={`rounded-full flex items-center justify-center transition-transform active:scale-90 ${
              isMainApp
                ? 'w-14 h-14 bg-accent text-primary font-bold text-[11px]'
                : 'w-12 h-12 bg-surface-sel'
            }`}>
              <Icon />
            </div>
